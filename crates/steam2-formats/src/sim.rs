...
use steam2_crypto::aes::{cbc_decrypt, ecb_decrypt_block};
use steam2_hash::crc32::crc32;
use steam2_core::reader::{cstr_at, read_u32_at as read_u32, read_u64_at as read_u64};

use crate::cp1252;
use crate::filter::Filter;
use steam2_compression::inflate::raw_inflate;
...
impl SimFile {
    fn parse(data: &[u8]) -> Result<SimFile, String> {
        if data.len() < 16 {
            return Err("sim: file too small".to_string());
        }
        let magic = read_u32(data, 0);
        if magic != SIM_MAGIC {
            return Err("sim: bad magic".to_string());
        }
        ...
        if data.len() < str_table_end + 8 {
            return Err("sim: file too small for string table".to_string());
        }
        ...
        if row_count > data.len().saturating_sub(rows_start) / 32 {
            return Err("sim: implausible row count".to_string());
        }
        let mut rows = Vec::with_capacity(row_count);
        for i in 0..row_count {
            let off = rows_start + i * 32;
            if data.len() < off + 32 {
                return Err("sim: table row out of bounds".to_string());
            }
            rows.push(SimRow {
    ...
    fn row_path(&self, row: &SimRow) -> Result<PathBuf, String> {
        let path_str = cp1252::decode_string(cstr_at(&self.string_table, row.path_str_off as usize)?);
        let file_str = cp1252::decode_string(cstr_at(&self.string_table, row.file_str_off as usize)?);
...
fn find_sid_files(sim_path: &Path) -> Result<Vec<Option<PathBuf>>, String> {
    ...
    let stem = sim_path
        .file_stem()
        .ok_or("sid: invalid .sim filename")?
        .to_string_lossy()
        .into_owned();
    ...
    let has_bare = entries.iter().any(|(name, _)| *name == format!("{stem_lc}.sid"));

    let mut slots: Vec<Option<PathBuf>> = Vec::new();
    if has_bare {
        for (name, path) in &entries {
            let Some(rest) = name.strip_prefix(&stem_lc).and_then(|r| r.strip_suffix(".sid")) else {
                continue;
            };
            let idx = if rest.is_empty() { Some(0) } else { rest.parse::<usize>().ok().and_then(|n| n.checked_sub(1)) };
            if let Some(idx) = idx {
        ...
        for (name, path) in &entries {
            let Some(rest) = name.strip_prefix(&prefix).and_then(|r| r.strip_suffix(".sid")) else {
                continue;
            };
    ...
    if slots.iter().all(Option::is_none) {
        return Err(format!("sid: no .sid files found next to {}", sim_path.display()));
    }
    Ok(slots)
}
...
impl SidStream {
    fn new(disks: Vec<Vec<Option<PathBuf>>>) -> SidStream {
        SidStream { disks, file: None, file_len: 0, disk_no: 0, sid_idx: 0 }
    }

    fn open(&mut self, disk_no: u8, sid_idx: usize) -> Result<(), String> {
        let disk = self
            .disks
            .get(disk_no.wrapping_sub(1) as usize)
            .ok_or_else(|| format!("sid: missing disk {disk_no}"))?;
        let path = disk
            .get(sid_idx)
            .and_then(|p| p.as_ref())
            .ok_or_else(|| format!("sid: missing .sid index {sid_idx} on disk {disk_no}"))?;
        let file = File::open(path).map_err(|e| e.to_string())?;
        self.file_len = file.metadata().map_err(|e| e.to_string())?.len();
        self.file = Some(file);
    ...
    fn ensure_open(&mut self, disk_no: u8, sid_idx: usize) -> Result<(), String> {
    ...
    fn advance(&mut self) -> Result<(), String> {
        ...
        if self.sid_idx + 1 < disk_len {
            ...
            Err("sid: ran out of .sid files while reading a block".to_string())
        }
    }

    fn seek(&mut self, pos: u64) -> Result<(), String> {
        self.file.as_mut().unwrap().seek(SeekFrom::Start(pos)).map_err(|e| e.to_string())?;
        Ok(())
    }

    fn read_exact(&mut self, buf: &mut [u8]) -> Result<(), String> {
        self.file.as_mut().unwrap().read_exact(buf).map_err(|e| e.to_string())?;
        Ok(())
    }
}
...
fn read_block_header(stream: &mut SidStream) -> Result<BlockHeader, String> {
    let mut buf = [0u8; 8];
    ...
    let flags = buf[7];
    if flags > 3 {
        return Err(format!("sid: unexpected block flags {}", flags));
    }
    if pad_size as u32 > size {
        return Err("sid: pad size larger than block size".to_string());
    }
    Ok(BlockHeader { size, pad_size, flags })
}
...
fn parse_pk_header(data: &[u8]) -> Result<PkLocalHeader, String> {
    if data.len() < 30 {
        return Err("sid: block too small for PK local file header".to_string());
    }
    let signature = read_u32(data, 0);
    if signature != PK_LOCAL_MAGIC {
        return Err("sid: expected PK local file header".to_string());
    }
    ...
    if data.len() < header_len {
        return Err("sid: PK header fields exceed block size".to_string());
    }
    Ok(PkLocalHeader { crc32, header_len })
}

fn extract_entry(stream: &mut SidStream, row: &SimRow, key: &[u8; 16]) -> Result<Vec<u8>, String> {
    ...
    while (out.len() as u64) < row.file_size {
        ...
        let real_len = (hdr.size as usize)
            ...
            .ok_or("sid: pad size exceeds block size")?;
        if real_len > payload_len {
            return Err("sid: pad size too small for encrypted block".to_string());
        }
        ...
        if compressed {
            ...
            if crc32(0, &inflated) != pk.crc32 {
                return Err("sid: CRC32 mismatch while decompressing block".to_string());
            }
    ...
    if out.len() as u64 != row.file_size {
        return Err("sid: output size doesn't match advertised file size".to_string());
    }
...
fn process_row(
    ...
    let key = keys.resolve(row.depot).unwrap_or_else(|| {
        if warned_depots.insert(row.depot) {
            eprintln!("no known key for depot {}; some depots use an all-zero key, trying that", row.depot);
        }
        [0u8; 16]
    });
    ...
    let mut out_file = File::create(&final_path).map_err(|e| e.to_string())?;
