...
impl<'a> Parser<'a> {
    ...
    fn parse_pattern(&mut self) -> Result<Vec<Node>, String> {
    ...
    fn parse_atom(&mut self) -> Result<Node, String> {
        match self.next() {
            Some('(') => {
                ...
                if self.next() != Some(')') {
                    return Err("regex: unclosed group".to_string());
                }
                Ok(Node::Group(inner))
            }
            ...
            Some('$') => Ok(Node::End),
            Some('\\') => {
                let c = self.next().ok_or("regex: trailing backslash")?;
                Ok(escape_node(c))
            }
            Some(c) => Ok(Node::Char(c)),
            None => Err("regex: unexpected end of pattern".to_string()),
        }
    }
    ...
    fn parse_class(&mut self) -> Result<Node, String> {
        ...
        loop {
            match self.peek() {
                None => return Err("regex: unclosed character class".to_string()),
                Some(']') if !first => {
                    self.next();
                    break;
                }
    ...
    fn parse_class_char(&mut self) -> Result<char, String> {
        match self.next() {
            Some('\\') => self.next().map(unescape_char).ok_or("regex: trailing backslash in class".to_string()),
            Some(c) => Ok(c),
            None => Err("regex: unclosed character class".to_string()),
        }
    }
}
...
fn node_positions(node: &Node, text: &[char], pos: usize) -> Vec<usize> {
    match node {
        Node::Char(c) => {
            if text.get(pos) == Some(c) { vec![pos + 1] } else { vec![] }
        }
        Node::Any => {
            if pos < text.len() { vec![pos + 1] } else { vec![] }
        }
        Node::Class(ranges, negate) => {
            if pos < text.len() && class_matches(ranges, *negate, text[pos]) { vec![pos + 1] } else { vec![] }
        }
        Node::Start => {
            if pos == 0 { vec![pos] } else { vec![] }
        }
        Node::End => {
            if pos == text.len() { vec![pos] } else { vec![] }
        }
...
impl Regex {
    pub fn new(pattern: &str) -> Result<Regex, String> {
        ...
        if parser.pos != chars.len() {
            return Err("regex: unexpected ')' in pattern".to_string());
        }
