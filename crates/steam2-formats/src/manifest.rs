...
use steam2_core::reader::{cstr_at, read_u32_at as read_u32};
use steam2_hash::adler32::adler32;
...
impl Manifest {
    pub fn parse(data: &[u8]) -> Result<Manifest, String> {
        if data.len() < HEADER_SIZE {
            return Err("manifest: too small for header".to_string());
        }
        let mst_version = read_u32(data, 0);
        if mst_version != 3 && mst_version != 4 {
            return Err("manifest: version isn't 3 or 4".to_string());
        }
        let binary_size = read_u32(data, 24);
        if binary_size as usize != data.len() {
            return Err("manifest: manifest size is wrong".to_string());
        }
        ...
        if checksum != computed {
            return Err("manifest: checksum is wrong".to_string());
        }

        let num_of_nodes = read_u32(data, 12);
        let header = Header { app_id: read_u32(data, 4), ver_id: read_u32(data, 8) };

        let nodes_start = HEADER_SIZE;
        let nodes_end = nodes_start + NODE_SIZE * num_of_nodes as usize;
        if nodes_end > data.len() {
            return Err("manifest: node table out of bounds".to_string());
        }
        ...
        for node in &nodes {
            ...
            while current.parent != 0xffffffff {
                hops += 1;
                if hops > nodes.len() {
                    return Err("manifest: cyclic node parent chain".to_string());
                }
                ...
                if parent_idx >= nodes.len() {
                    return Err("manifest: node parent index out of bounds".to_string());
                }
        ...
        Ok(Manifest { header, nodes, id_to_path })
    }
}
...
impl CompressionType {
    pub fn from_u8(v: u8) -> Result<CompressionType, String> {
        match v {
            ...
            _ => Err(format!("manifest: invalid compression type {}", v)),
        }
    }
}
...
mod tests {
    ...
    fn build_manifest(nodes: &[(u32, u32, u32, u32)], string_table_bytes: &[u8]) -> Vec<u8> {
        ...
        data[string_table..string_table + string_table_bytes.len()].copy_from_slice(string_table_bytes);
