...
impl KeySource {
    pub fn add(&mut self, spec: &str) -> Result<(), String> {
        match spec.split_once(':') {
            Some((depot, hex)) => {
                let depot: u32 = depot.parse().map_err(|_| "--key: depot must be an integer".to_string())?;
    ...
    pub fn add_file(&mut self, path: &str) -> Result<(), String> {
        self.entries.extend(keysfile::parse(path)?.into_iter().map(|(d, k)| (Some(d), k)));
        Ok(())
    }
    ...
    pub fn resolve(&self, depot: u32) -> Option<[u8; 16]> {
        self.entries
            ...
            .map(|(_, k)| *k)
            .or_else(|| database::lookup_key(depot))
    }
}
